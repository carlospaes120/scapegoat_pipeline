...
p4_resumo = spark.sql("""
  ...
  pico AS (
    SELECT caso, papel, dias_desde_estopim AS dia_pico_papel
    FROM (SELECT *, ROW_NUMBER() OVER (PARTITION BY caso, papel ORDER BY mencoes DESC, dias_desde_estopim) AS rn FROM por_papel_dia)
    WHERE rn = 1
  ),
  alvo AS (
    SELECT g.caso, SUM(g.peso) AS mencoes_alvo,
           MAX_BY(g.dias_desde_estopim, g.peso) AS dia_pico_alvo_menc
    FROM (SELECT caso, dias_desde_estopim, SUM(peso) AS peso FROM scapegoat.gold.grafo_arestas WHERE papel_destino = 'alvo' GROUP BY caso, dias_desde_estopim) g
    GROUP BY g.caso
  )
  SELECT pp.caso, pp.papel,
         COUNT(DISTINCT p.conta_id)                         AS contas,
         SUM(pp.mencoes)                                    AS mencoes,
         ROUND(SUM(pp.mencoes) / MAX(t.mencoes_caso), 3)    AS parcela_mencoes_caso,
         ROUND(SUM(pp.mencoes) / MAX(a.mencoes_alvo), 2)    AS razao_vs_alvo,
         MAX(pk.dia_pico_papel)                             AS dia_pico_papel,
         MAX(pk.dia_pico_papel) - 1                         AS defasagem_vs_pico_calendario,
         MAX(a.dia_pico_alvo_menc)                          AS dia_pico_mencoes_alvo
  FROM por_papel_dia pp
  JOIN scapegoat.gold.papel_narrativo_v0 p ON p.caso = pp.caso AND p.papel_inicial = pp.papel
  JOIN tot t  ON t.caso = pp.caso
  JOIN alvo a ON a.caso = pp.caso
  JOIN pico pk ON pk.caso = pp.caso AND pk.papel = pp.papel
  GROUP BY pp.caso, pp.papel
  ORDER BY pp.caso, mencoes DESC
""")
display(p4_resumo)
p4_resumo.toPandas().to_csv(f"{SAIDA}/p4_resumo.csv", index=False)
