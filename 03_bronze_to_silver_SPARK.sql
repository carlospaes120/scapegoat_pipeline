...
-- VERSÃO SPARK SQL/DELTA
-- Projeto: scapegoat-pipeline · Autor: Carlos A. Paes da Silva
-- Plataforma: Databricks Free Edition (serverless) · Spark SQL
--
-- TRADUÇÃO DE POSTGRESQL PARA SPARK SQL (04/09/2026)
-- Mudanças principais:
--   · Regex de hashtags: #\w+ → #[\p{L}\p{N}_]+ (Unicode)
--   · Demais sintaxe é 99% compatível
--
...
-- DISCLAIMER: Este script foi TRADUZIDO de PostgreSQL. Não mexer em:
-- · Nomes de colunas
-- · Comentários (em português, melhor que em inglês)
-- · Ordem das operações no QC
-- Validar com EXPLAIN PARSED antes de primeira execução.

-- =====================================================================
-- 0. Schemas — a arquitetura medalhão do enunciado
-- =====================================================================
...
-- =====================================================================
-- 1. BRONZE (sem mudanças — já existe)
-- =====================================================================
...
--
-- ⭐ AQUI ENTRA A LIMPEZA DE TEXTO (texto_limpo)
CREATE OR REPLACE VIEW silver.v_deduplicado
COMMENT 'Uma linha por postagem. Contadores = máximo observado entre as capturas do mesmo id. Inclui texto_limpo validado contra 1.569 pares.'
AS
SELECT
  caso_slug, id_nativo,
  ...
  -- ⭐ LIMPEZA DE TEXTO — validada contra 1.569 pares (100% reprodução)
  -- Operações em ordem: URLs → Menções → Hashtags → Espaços múltiplos
  -- Regex de hashtags usa [\p{L}\p{N}_]+ para capturar acentos (Unicode)
  trim(regexp_replace(
    regexp_replace(
      regexp_replace(
        regexp_replace(min_by(texto, linha),
...
USING (
  SELECT d.id_nativo, d.caso_slug, c.conta_id AS autor_conta_id, d.created_at,
         d.texto, d.texto_limpo, d.idioma, d.tipo_ref, d.ref_status_id,
...
WHEN NOT MATCHED THEN INSERT (
  plataforma, id_nativo, caso_slug, autor_conta_id, created_at, texto, texto_limpo, idioma,
...
VALUES (
  ...
  origem.texto, origem.texto_limpo, origem.idioma, origem.tipo_ref, origem.ref_status_id,
...
--
-- · texto_limpo é DERIVADO e reutilizável; não precisa ser persistido
--   separadamente em todas as tabelas.
--
-- · MUDANÇAS SPARK SQL (04/09/2026):
--   ✅ Regex de hashtags: #\w+ → #[\p{L}\p{N}_]+  (Unicode, 1.569/1.569 pares)
--   ✅ Demais cláusulas 100% compatíveis com PostgreSQL
--   ✅ MERGE, LATERAL VIEW, from_json todos nativos no Spark
-- =====================================================================
